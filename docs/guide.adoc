...
HyprSpace (the enhanced fork of AeroSpace) looks for its configuration file in the following locations:

. `~/.hyprspace.toml` (primary location for HyprSpace)
...
**Important:** HyprSpace uses `~/.hyprspace.toml` as its primary configuration file, not `~/.aerospace.toml`.
This distinguishes it from the upstream AeroSpace project.
...
[source,shell]
----
cp /Applications/AeroSpace.app/Contents/Resources/default-config.toml ~/.hyprspace.toml
----
...
This section provides detailed documentation for key configuration options available in HyprSpace.
...
Controls whether HyprSpace automatically starts when you log into macOS.
...
This is the recommended way to ensure HyprSpace is always running. Alternative methods like using `launchd` or adding to Login Items manually are also possible but less integrated.
...
Specifies commands to run automatically after HyprSpace starts.
...
The dwindle layout (HyprSpace exclusive) offers advanced customization options:
...
===== Single Window Aspect Ratio (HyprSpace Exclusive)
...
HyprSpace supports sophisticated per-monitor gap configurations using an array syntax. This allows different gap sizes for different monitors.
...
== Centered Workspace Bar (HyprSpace Exclusive Feature)
