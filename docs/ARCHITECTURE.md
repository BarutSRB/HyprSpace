...
- **`IntentLedger`** is a `@MainActor` ring buffer (capacity 256) of `Intent` records. `IntentKind` has exactly six cases: `activateApp`, `appRevealFocus`, `focusPolicyLease`, `focusWindow`, `replacementFocus`, `sameAppCloseProbe`. `appRevealFocus` carries the exact window-handle identity, app-visibility generation, focus watermark and fingerprint, plus a normal-window or scratchpad destination so unhide completion cannot focus stale state. The intent is created before calling `NSRunningApplication.unhide()`; once a live application is resolved, that method's Boolean result is diagnostic rather than terminal, and authoritative `didUnhide`, termination, or the two-second deadline decides the outcome. Each record carries the global intake `seq` at issue time, a lifecycle `phase` (`pending`/`confirmed`/`superseded`/`expired`/`cancelled`), and retry state.
